/**
@see https://github.com/keel/aes-cross/blob/master/typescript/AES.ts
Pure typescript AES

适配微信/企业微信的 AES 加解密。微信使用 AES-256-CBC，但 padding 不是标准的 PKCS7。Cloudflare Worker 提供的加解密 API 不支持自定义 padding
*/
...
export class AES {
  ...
  constructor(key: Uint8Array) {
    ...
    this.KEY = new Array<number>(AES.AES_NB * (this.NR + 1) * 4)
    this.expandKey(key)
  ...
  static pad(text: Uint8Array): Uint8Array {
    const blockNumber = Math.ceil((text.length + 1) / 32)
    const paddedMessageLength = blockNumber * 32
    const remainedLength = paddedMessageLength - text.length
    const paddedMessage = new Uint8Array(paddedMessageLength)
    paddedMessage.set(text, 0)
    paddedMessage.set(
  ...
  static unpad(text: Uint8Array): Uint8Array {
    const lastByte = text[text.length - 1]
    if (lastByte < 1 || lastByte > 32) {
      return text
    }
    return new Uint8Array(text.slice(0, text.length - lastByte))
  }

  encrypt(text: Uint8Array) {
    text = AES.pad(text)
    const output = new Uint8Array(text.length)
    let prev = this.iv
    for (let i = 0; i < text.length; i += AES.blockSize) {
      prev = this._encrypt(AES.xor(text.slice(i, i + AES.blockSize), prev))
      output.set(prev, i)
    }
    return output
  }

  decrypt(text: Uint8Array) {
    const output = new Uint8Array(text.length)
    let prev = this.iv
    for (let i = 0; i < text.length; i += AES.blockSize) {
      const t = text.slice(i, i + AES.blockSize)
      output.set(AES.xor(this._decrypt(t), prev), i)
      prev = t
    }
  ...
  private _encrypt(block: Uint8Array): Uint8Array {
  ...
  private _decrypt(block: Uint8Array): Uint8Array {
    const state = this.createState(block)
  ...
  private createState(block: Uint8Array): Uint8Array {
    const state = new Uint8Array(4 * AES.AES_NB)
  ...
  private stateToResult(state: Uint8Array): Uint8Array {
    const result: Uint8Array = new Uint8Array(16)
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < AES.AES_NB; j++) {
